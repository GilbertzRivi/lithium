...
**Bezpieczeństwo hybrydowe.** `base_key` jest wyprowadzony z obu gałęzi: `ecdh_key` (ścieżka X25519) oraz `seed_plain` (odzyskany przez ML-KEM). Konkretnie: `seed_plain` jest saltem HKDF w derywacji `base_key`, a `ecdh_key` jest IKM. Żeby obliczyć `base_key` bez obu wejść, atakujący musiałby złamać X25519 (żeby obliczyć `ecdh_key` bez klucza prywatnego) lub ML-KEM-1024 (żeby odzyskać `seed_plain` bez klucza prywatnego) — o ile HKDF-SHA256 nie ma słabości pozwalającej pominąć jedno z wejść. Obie gałęzie są niezależne i według założeń konstrukcji obie są konieczne.
...
**Ochrona przed replay.** Zarejestrowany prawidłowy `WirePayload` może zostać ponownie przesłany do tego samego odbiorcy. AEAD zakończy się sukcesem. Co istotne, sam `session.rs` **również nie odrzuca** powtórki na poziomie kryptograficznym: klucz RX nie jest konsumowany przy użyciu (lookup `self_get_rx_privs` to odczyt, nie usunięcie), a numery sekwencyjne (`ack_seq` i okno RX 32) sterują **wyłącznie cyklem życia kluczy i forward secrecy**, nie detekcją replay — powtórka w obrębie aktualnego okna deszyfruje się ponownie.

Wykrywanie replay realizowane jest **wyżej, w warstwie przechowywania** (`lithiumd/src/traffic.rs` + `db`): każda wiadomość niesie w **podpisanym** nagłówku losowy `msg_id` (16 B), a auto-fetch (`traffic.rs`) zapisuje ją przez `add_message` do tabeli z ograniczeniem `UNIQUE(msg_id)`. Powtórzony `msg_id` zwraca `Ok(false)` → element oznaczany `duplicate`, niezapisywany do historii i niepokazywany. Dodatkowo serwer kasuje wiadomość atomowo przy pierwszym pobraniu (one-time fetch), więc do realnego replay potrzebny jest złośliwy serwer re-injektujący ramkę do skrzynki. Ponowna deszyfracja takiej powtórki ma efekty uboczne idempotentne (numery sekwencji i generacje przesuwają się tylko w przód, brak regresji stanu).
...
Nie zapewnia uwierzytelnienia, ochrony przed replay ani forward secrecy w obrębie jednej epoki ratchet na własną rękę. Uwierzytelnienie (dual-sign) i forward secrecy są realizowane w warstwie sesji (`lithiumd/src/e2e/session.rs`); ochrona przed replay leży natomiast **nie** w `session.rs`, lecz w warstwie przechowywania — deduplikacja po `msg_id` z ograniczeniem `UNIQUE` (auto-fetch w `traffic.rs` + `add_message`), wsparta one-time fetch po stronie serwera.
