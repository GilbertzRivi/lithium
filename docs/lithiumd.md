...
Kroki:
1. Walidacja `data_password` przez `PasswordPolicy` (min 12 znaków, wielkie/małe/cyfry/znaki specjalne, bez spacji).
...
4. Inicjalizuje `PasswordFileMkProvider` z plikiem `{data_dir}/keystore/user/mk.enc`.
...
2. Wykonuje rejestrację OPAQUE (dwufazową): `client_registration_start` → POST `/user/register/start`, potem `client_registration_finish` → POST `/user/register/finish`. Serwer nigdy nie widzi hasła konta ani jego hasha — zapisuje wyłącznie rekord OPAQUE.
3. Owija DEK pod `export_key` z OPAQUE (`wrap_dek_under_export_key`) i przekazuje go serwerowi jako nieprzejrzysty blob (serwer go nie używa, zwraca przy logowaniu).
...
4. Inicjalizuje `DataManager` z SQLite (`{data_dir}/storage/lithiumd.sqlite`).
...
Krok 1 z 4 parowania commit-reveal. Tworzy **commitment** dla nowego lub istniejącego kontaktu — surowy kod `lci1:` nie opuszcza daemona na tym etapie.
...
Odpowiedź zawiera `contact_id` (hex) i `commitment` (`SHA256("lithiumd/pair-commit/v1" || kod)`, hex). Pełny czterokrokowy flow opisuje [ipc-reference.md](ipc-reference.md).

---

### `AcceptCommitment` / `RevealInvite` / `FinalizePairing` — wymagają auth + storage

Kroki 2–4 parowania commit-reveal; kolejność wymusza daemon. Pełne pola żądań/odpowiedzi i kody błędów — [ipc-reference.md](ipc-reference.md).

- **`accept_commitment`** (B, krok 2): zapisuje `commitment` twórcy (A), generuje własny `contact_id` i klucze, zwraca własny kod (`code`, `lci1:`) do odesłania A.
- **`reveal_invite`** (A, krok 3): przyjmuje kod B (`peer_code`), ustawia peera na tożsamość B, ujawnia własny kod (`code`) do odesłania B. Własny kod jest emitowany dopiero po podaniu kodu peera.
- **`finalize_pairing`** (B, krok 4): przyjmuje kod A (`peer_code`), weryfikuje `ct_eq(SHA256("lithiumd/pair-commit/v1" || kod_A), pending_commit)` i ustawia peera na tożsamość A. Niezgodność → `commitment_mismatch`.

Po kroku 4 obie strony mają `peer_set == true`.
...
Kroki (szczegóły: [crypto-protocol.md](crypto-protocol.md)):
1. Każda strona liczy „party transcript" — `HKDF` po konkatenacji 8 pól tożsamości (`cid`, `x_pub`, `ed_pub`, `dili_pub`, `k_pub` + 3 klucze mailbox) pod etykietą `"lithiumd/party-transcript/v1"`. Transkrypty obu stron są sortowane → `t_a`, `t_b`.
2. `shared = ECDH(self_x_priv, peer_x_pub)`.
3. `sas32 = HKDF(shared, info="lithiumd/contact-verify-emoji/v1" || t_a || t_b)` → 32 bajty.
4. `emoji[i] = TABLE[sas32[i] mod 64]` dla `i = 0..6` (tablica 64 symboli).
...
w tym plaintextu i kluczy. Binding tokenu do UID/PID (Linux) ogranicza ryzyko — patrz [security-model.md](security-model.md).