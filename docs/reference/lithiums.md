...
Providery master key (`LITHIUM_MK_PROVIDER`) i zmienne TPM opisuje [deploy-instructions.md](../operations/deploy-instructions.md).
...
Serwer implementuje dwa tryby transportu opisane w [crypto-protocol.md](../protocol/crypto-protocol.md#warstwa-transportu-daemonserwer) — **Shake** (inicjalizacja sesji, efemeryczne klucze klienta w nagłówkach, TTL 60 s) oraz **Session** (po Shake, identyfikatory `ses-x`/`ses-k`, TTL 120 s). Specyfika serwera: klucze prywatne sesji trzymane są w `EphemeralStoreManager` pod losowymi identyfikatorami `ses-x`/`ses-k`, a po każdej odpowiedzi generowane są nowe pary (rolling session).
...
Globalny model „serwer jako wrogie relay" (co serwer widzi per request, co jest przed nim chronione) opisuje [security-model.md](../security/security-model.md). Mechanizmy specyficzne dla serwera, które go realizują:

- **Anti-replay** — SHA256 ciała przechowywany 600 s; okno timestampu ±60 s odrzuca powtórki i nieświeże żądania.
- **Jednorazowy JWT** — token zużywany przy użyciu (`store.take`), bez replay uwierzytelnionych żądań.
- **Nieprzejrzyste `sub`** — `HMAC-SHA256(user_id, random_seed)` zamiast surowego ID w JWT.
- **Izolacja pól DB** — każde pole użytkownika pod osobną AAD; DEK z błędną AAD nie odszyfruje innego pola.
- **Dopełnianie rozmiaru** — ciała i nagłówki odpowiedzi paddowane do bloków, by ukryć długości.

Czego serwer nie ma i nie widzi: kluczy prywatnych klientów, plaintextu treści (warstwa E2E), ani — po jednorazowym pobraniu lub restarcie — `msg_key` (zaległe wiadomości stają się wtedy trwale nieodszyfrowalne).
