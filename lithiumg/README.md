...
├── app.rs    — logika UI: maszyna stanów ekranów, rendering, obsługa zdarzeń
...
Wątek główny odpytuje kanał co klatkę renderowania (co ~100ms w ekranie Ready).
...
- `[Fetch]` — pobiera nowe wiadomości z serwera przez daemona
- `[Reload]` — przeładowuje historię z lokalnej bazy
- `[Forget]` — usuwa kontakt
...
| `contact_fetch`        | Pobierz nowe wiadomości z serwera | `contact_id`                         | `messages[{ok,err}]`    |
| `create_invite`        | Utwórz zaproszenie                | `contact_id` (opt.)                  | `contact_id`, `code`    |
| `accept_invite`        | Przyjmij zaproszenie              | `code`, `label`, `contact_id` (opt.) | `contact_id`, `my_code` |
...
CreateInviteResult { contact_id: String, code: String }
AcceptInviteResult { contact_id: String, my_code: String }
...
Zaproszenia działają dwukierunkowo — każda strona generuje własny kod i wymienia się nim z drugą stroną:
...
Przekaż kod_A Stronie B (out-of-band: chat, email, itp.)
...
Wybierz kontakt + wklej kod_B
[Reply]
  → accept_invite(kod_B, contact_id=A)

Teraz obie strony mają peer_set=true
...
- Po `create_invite` / `accept_invite` → automatycznie zaznaczany nowy kontakt
...
Po pobraniu wiadomości (`contact_fetch`) wynik jest skracany do komunikatu statusowego, np.:
```
Fetch complete: received=3, errors=1 [decrypt_failed=1]
```