...
| `password_hash` | `BYTEA`    | String PHC Argon2id, zaszyfrowany DEK serwera                              |
...
Pola ciała żądania:
- `handler` — nazwa użytkownika (przycinana, małe litery przy przechowywaniu)
- `password` — hasło w plaintekście (hashowane po stronie serwera przez Argon2id)
- `dek` — szesnastkowo zakodowany blob DEK klienta (nieprzejrzysty dla serwera;
  przechowywany zaszyfrowany i zwracany przy logowaniu)

Nagłówki żądania (poza nagłówkami transportowymi): `key-ed`, `key-dili` 
(długoterminowe klucze publiczne podpisujące klienta)

Działanie:
1. Sprawdza limit rejestracji
2. Waliduje, że `dek` jest poprawnym hex
3. Wywołuje `create_user`; jeśli handler już istnieje, inkrementuje licznik niepowodzeń 
  i zwraca sukces (brak wycieku enumeracji handlerów)
...
Tryb auth `LoginByHandler` ładuje `UserRecord` z DB przed uruchomieniem handlera.

Działanie:
1. Sprawdza limit logowania
2. Weryfikuje `password` względem przechowywanego hashu PHC Argon2id
3. Przy niepowodzeniu: inkrementuje licznik, zwraca `401 invalid_credentials`
4. Przy sukcesie: zeruje licznik, wystawia JWT przez `reply_ok_authed(session_ttl=120)`

Ciało odpowiedzi: `{"msg": "Ok", "dek": "<client_dek_hex>", "tok_hex": "<jwt_hex>"}`
oraz nowe pary kluczy sesji w nagłówkach odpowiedzi.

Pole `dek` zawiera DEK po stronie klienta zarejestrowany wcześniej — serwer przechowuje go i zwraca, lecz nigdy go nie używa.

### `POST /msg/send`

Wymaga auth JWT (tryb `JwtUser`).
...
| 401 `invalid_credentials` | Błędne hasło lub brakujący JWT               |
