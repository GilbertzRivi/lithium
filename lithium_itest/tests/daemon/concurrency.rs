...
async fn test_parallel_pings_both_succeed() {
    ...
    let t1 = tokio::spawn(async move {
        IpcClient::connect(&sp1).await.send(json!({"cmd": "ping"})).await
    });
    let t2 = tokio::spawn(async move {
        IpcClient::connect(&sp2).await.send(json!({"cmd": "ping"})).await
    });
...
async fn test_parallel_auth_token_use() {
    ...
    let t1 = tokio::spawn(async move {
        IpcClient::connect(&sp1).await.send(json!({"cmd": "wipe_local", "auth_token": tok1})).await
    });
    let t2 = tokio::spawn(async move {
        IpcClient::connect(&sp2).await.send(json!({"cmd": "ping", "auth_token": tok2})).await
    });
...
async fn test_shutdown_drops_sibling_connection() {
    ...
    ca.send(json!({"cmd": "set_server_url", "url": "http://127.0.0.1:19999"})).await;
    let r = ca.send(json!({"cmd": "unlock_keystore", "data_password": DATA_PASS})).await;
    let tok = r["result"]["ipc_auth_token"].as_str().unwrap().to_owned();

    let mut cb = IpcClient::connect(&d.socket_path).await;
    assert!(cb.send(json!({"cmd": "ping"})).await["ok"].as_bool().unwrap());

    let r = ca.send(json!({"cmd": "shutdown", "auth_token": tok})).await;
    assert!(r["ok"].as_bool().unwrap());

    tokio::time::sleep(Duration::from_millis(300)).await;
    assert!(cb.try_read_line().await.is_none(), "sibling connection should be closed after shutdown");
}