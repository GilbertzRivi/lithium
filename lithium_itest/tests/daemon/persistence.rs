...
async fn test_server_url_survives_restart() {
    ...
    assert!(c2.send(json!({"cmd": "ping"})).await["result"]["status"]["has_server_url"].as_bool().unwrap());
}
...
async fn test_server_identity_survives_restart() {
    ...
    let d2 = DaemonProcess::start_in(data_dir.path()).await;
    let mut c2 = IpcClient::connect(&d2.socket_path).await;
    assert!(c2.send(json!({"cmd": "ping"})).await["result"]["status"]["has_server_identity"].as_bool().unwrap());
}
...
async fn test_registered_state_after_restart() {
    ...
    let d2 = DaemonProcess::start_in(data_dir.path()).await;
    let mut c2 = IpcClient::connect(&d2.socket_path).await;
    let r = c2.send(json!({"cmd": "unlock_keystore", "data_password": DATA_PASS})).await;
    assert!(r["ok"].as_bool().unwrap(), "{:?}", r);

    let ping = c2.send(json!({"cmd": "ping"})).await;
    assert!(ping["result"]["status"]["is_registered_on_disk"].as_bool().unwrap());
    assert!(!ping["result"]["status"]["needs_register"].as_bool().unwrap());
    assert!(!ping["result"]["status"]["has_credentials"].as_bool().unwrap());
    assert_eq!(ping["result"]["ui_state"].as_str().unwrap(), "needs_credentials");
}
...
async fn test_pending_invite_contact_survives_restart() {
    ...
    let us = c2.send(json!({"cmd": "unlock_storage", "auth_token": tok2})).await;
    assert!(us["ok"].as_bool().unwrap(), "{:?}", us);

    let list = c2.send(json!({"cmd": "contacts_list", "auth_token": tok2})).await;
...
async fn test_received_message_survives_restart() {
    ...
    let acc_b = cb.send(json!({"cmd": "accept_invite", "code": code_a, "label": "A", "auth_token": tok_b})).await;
    ...
    let r = cb2.send(json!({"cmd": "unlock_keystore", "data_password": DATA_PASS})).await;
    assert!(r["ok"].as_bool().unwrap(), "{:?}", r);
    ...
    let us = cb2.send(json!({"cmd": "unlock_storage", "auth_token": tok_b2})).await;
    assert!(us["ok"].as_bool().unwrap(), "{:?}", us);

    let msgs = cb2.send(json!({"cmd": "messages_list", "contact_id": cid_b, "auth_token": tok_b2})).await;
...
async fn test_wipe_local_then_restart_is_clean() {
    ...
    c.send(json!({"cmd": "wipe_local", "auth_token": tok})).await;
    drop(d);
